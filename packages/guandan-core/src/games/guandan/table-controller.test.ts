...
test("首局由南座领出，且后续按逆时针进入东座", () => {
  ...
  expect(submitTableAction(game, opening)).toMatchObject({
    ok: true,
    state: { current: "east" },
  });
});
...
test("机器人从合法动作中枚举并压制三带二", () => {
  ...
  const game: TableGame = {
    cardsById: new Map(
      [...southCards, ...westCards].map((item) => [item.id, item]),
    ),
    state: {
      hands: {
        east: ["east-other"],
  ...
  expect(
    chooseTableBotAction({ ...game, state: westTurn.state }),
  ).toMatchObject({
    type: "play",
    actor: "west",
...
test("机器人跟炸时使用完整更大炸弹，不拆五张炸为四张", () => {
  ...
  const game: TableGame = {
    cardsById: new Map(
      [...southCards, ...eastCards].map((item) => [item.id, item]),
    ),
    state: {
      hands: {
...
test("机器人可以用完整炸弹压制不同牌型", () => {
  const southCards = [
    card("south-aa", "A", "spades"),
    card("south-ab", "A", "hearts"),
  ];
  const eastCards = [
...
test("normal strategy uses a matching three-with-pair instead of an unnecessary bomb", () => {
  ...
  const game: TableGame = {
    cardsById: new Map(
      [...southCards, ...westResponse, ...westBomb].map((item) => [
        item.id,
        item,
      ]),
    ),
  ...
  expect(lead).toMatchObject({
    interpretation: { type: "three-with-pair", comparisonKey: [4] },
  });
  if (!lead) return;
  ...
  expect(
    chooseTableBotAction({ ...game, state: afterNorthPass.state }),
  ).toMatchObject({
    type: "play",
    actor: "west",
...
test("normal-vNext blocks the next seat with the smallest sufficient joker and retains the big joker", () => {
  ...
  const game: TableGame = {
    cardsById: new Map(
      [eastAce, bigJoker, smallJokerOne, smallJokerTwo].map((item) => [
        item.id,
        item,
      ]),
    ),
  ...
  if (!afterLead.ok) return;
  expect(
    chooseTableBotAction({ ...game, state: afterLead.state }),
  ).toMatchObject({
    type: "play",
    actor: "north",
...
test("normal-vNext blocks the next seat with the smallest sufficient joker while retaining the big joker", () => {
  ...
  const game: TableGame = {
    cardsById: new Map(
      [eastNine, ...northStraight, smallJoker, bigJoker].map((item) => [
        item.id,
        item,
      ]),
    ),
