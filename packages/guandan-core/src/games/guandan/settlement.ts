...
export function settleSingleTribute(
  ...
  if (!canReturnTribute(returned.rank, level))
...
export function settleRound(input: SettlementInput): Settlement {
  ...
  if (
    input.antiTribute &&
    (!(input.proof?.length === 2) ||
      !input.proof.every((card) => card.rank === "big-joker"))
  )
  ...
  const delta = double
    ? 3
    : partner(input.finish[0]) === input.finish[2]
      ? 2
      : 1;
  const index = Math.min(
    levels.indexOf(input.level) + delta,
    levels.length - 1,
  );
