...
function compareRankGroups(
  ...
  return (
    rankOrder(right.key as Rank, levelRank) -
    rankOrder(left.key as Rank, levelRank)
  );
}
  ...
  const bombs = groups
    ...
    .sort(
      (left, right) =>
        ...
        compareRankGroups(levelRank, left, right),
    );
  return [...normal, ...bombs];
}
...
export function groupOrderedDisplayCards(
  ...
  for (const cardId of order) {
    ...
    const previousCard = previous
      ? cardsById.get(previous.cardIds.at(-1)!)
      : undefined;
    if (previous && previousCard?.rank === card.rank) {
      groups[groups.length - 1] = {
        ...previous,
        cardIds: [...previous.cardIds, cardId],
      };
      continue;
    }
    groups.push({
...
export function sortHumanDisplayCards(
  ...
  return groupHumanDisplayCards(hand, cardsById, levelRank).flatMap(
    (group) => group.cardIds,
  );
}
...
export function reconcileHumanDisplayOrder(
  ...
  const validPreferred = (preferredOrder ?? []).filter(
    (cardId, index, all) =>
      handIds.has(cardId) && all.indexOf(cardId) === index,
  );
