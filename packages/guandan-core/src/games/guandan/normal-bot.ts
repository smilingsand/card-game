...
function hasNaturalStraightContaining(
  hand: BotView["selfHand"],
  rank: string,
): boolean {
  const ranks = new Set(
    hand.filter((card) => card.suit !== "joker").map((card) => card.rank),
  );
...
function structuralBreakPenalty(action: TurnAction, view: BotView): number {
  if (action.type !== "play" || action.interpretation.type !== "single")
    return 0;
...
function avoidUnnecessaryBombPenalty(
  action: TurnAction,
  hasNonBombLegalPlay: boolean,
): number {
...
export function chooseNormalBotAction(
  ...
  if (view.highestSeat === undefined) {
    ...
    return action
      ? { action, score: 0, reasons: ["领出沿用结构化基线"] }
      : undefined;
  }
  const threat = Object.entries(view.remainingCardCounts).some(
    ([seat, count]) =>
      seat !== teammate[view.selfSeat] && seat !== view.selfSeat && count <= 1,
  );
  const phase =
    view.selfHand.length > 18
      ? "开局"
      : view.selfHand.length > 7
        ? "中局"
        : "残局";
  ...
  const ranked = view.legalActions
    .map((action) => {
      ...
      let score =
        action.type === "pass"
          ? 0
          : (action.interpretation.comparisonKey.at(-1) ?? 0);
      if (
        action.type === "pass" &&
      ...
      const unnecessaryBombPenalty = avoidUnnecessaryBombPenalty(
        action,
        hasNonBombLegalPlay,
      );
