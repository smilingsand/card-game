...
test("识别冻结的全部牌型并拒绝非法组合", () => {
  ...
  expect(
    recognizePatterns(cards(["3", "spades"], ["4", "clubs"]), "2"),
  ).toEqual({
    ok: false,
...
test("逢人配枚举解释且不改动实体牌", () => {
  ...
  expect(
    result.ok && result.interpretations.some((item) => item.type === "triple"),
  ).toBe(true);
...
test("三带二的比较键只取三张牌点，不受附属对子影响", () => {
  ...
  expect(result).toMatchObject({
    ok: true,
    interpretations: [
      expect.objectContaining({ type: "three-with-pair", comparisonKey: [7] }),
    ],
  });
});
...
test("级牌进入连续牌型时按普通点数比较，A2345 的 A 按最小端处理", () => {
  ...
  const interpretation = (
    result: ReturnType<typeof recognizePatterns>,
    type: string,
  ) => {
