...
export type PatternErrorCode =
  "empty_cards" | "duplicate_card_id" | "no_legal_pattern";
export interface PatternInterpretation {
  readonly type: PatternType;
  readonly cardIds: readonly string[];
  readonly wildcardAs: Readonly<
    Record<string, { readonly rank: Rank; readonly suit: Suit }>
  >;
...
export type PatternRecognition =
  | {
      readonly ok: true;
      readonly interpretations: readonly PatternInterpretation[];
    }
...
function classify(
  cards: readonly { rank: Rank; suit: Suit }[],
): PatternType | undefined {
  const ranks = cards.map((card) => card.rank);
  const counts = [
    ...new Map(
      ranks.map((rank) => [rank, ranks.filter((item) => item === rank).length]),
    ).values(),
  ].sort();
  ...
  if (
    cards.length === 4 &&
    cards.every(
      (card) => card.rank === "small-joker" || card.rank === "big-joker",
    )
  )
    return "four-jokers";
  ...
  if (cards.length === 6 && counts.join() === "3,3" && consecutive(ranks, 2))
    return "steel-plate";
  if (cards.length === 5 && consecutive(ranks, 5))
    return sameSuit ? "straight-flush" : "straight";
  if (cards.length >= 4 && cards.length <= 10 && counts.length === 1)
    return "normal-bomb";
  return undefined;
}
...
export function recognizePatterns(
  ...
  const visit = (
    ...
    const projected = [
      ...fixed,
      ...wildcards.map((card, i) => ({ ...card, ...assigned[i] })),
    ];
    const type = classify(projected);
    if (!type) return;
    const wildcardAs = Object.fromEntries(
      wildcards.map((card, i) => [card.id, assigned[i]]),
    );
    const key = `${type}:${JSON.stringify(wildcardAs)}`;
    if (
      !results.some(
        (item) => `${item.type}:${JSON.stringify(item.wildcardAs)}` === key,
      )
    )
      results.push({
        type,
