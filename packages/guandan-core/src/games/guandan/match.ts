...
export function levelForLeader(
  levels: MatchLevels,
  leader: Seat,
): MatchLevels[keyof MatchLevels] {
...
export function levelsAfterRound(
  levels: MatchLevels,
  ...
  const delta =
    finish[1] === teammate(finish[0])
      ? 3
      : finish[2] === teammate(finish[0])
        ? 2
        : 1;
...
function highestTributeCard(
  ...
  return [...hand]
    ...
    .sort(
      (a, b) => value(b, level) - value(a, level) || a.id.localeCompare(b.id),
    )[0];
}
...
export function createTributePlan(
  ...
  const proof = loserSeats
    .flatMap((seat) => bigJokers(hands[seat]))
    .slice(0, 2);
  const antiTribute = double
    ? proof.length >= 2 &&
      (bigJokers(hands[loserSeats[0]]).length >= 2 ||
        bigJokers(hands[loserSeats[1]]).length >= 1)
    : proof.length >= 2;
  if (antiTribute)
    return {
      kind: "none",
  ...
  const tributes = loserSeats
    ...
    .filter(
      (entry): entry is { from: Seat; card: Card } => entry.card !== undefined,
    );
  if (!double)
    return {
      kind: "single",
      ...
      proof: [],
      obligations: [
        { from: tributes[0].from, to: head, cardId: tributes[0].card.id },
      ],
    };
  const [high, low] = [...tributes].sort(
    (a, b) => value(b.card, level) - value(a.card, level),
  );
  return {
    kind: "double",
    antiTribute: false,
