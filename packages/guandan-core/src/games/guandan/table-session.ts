...
type TributeResolvedEvent = Event<{
  ...
  readonly hands: Readonly<
    Record<import("../../platform/types").Seat, readonly string[]>
  >;
}>;
type TableSessionEvent =
  | TableActionEvent
  | RoundStartedEvent
  | TributeProgressEvent
...
function snapshotState(
  session: Pick<TableSession, "game" | "match">,
): TableSnapshotState {
  return {
...
function applyEvent(state: ReplayState, event: TableSessionEvent): ReplayState {
  if (event.type === "round.started" && "match" in event.payload) {
    ...
    return {
      match,
      game: createTableGame(match.roundSeed, {
  ...
  if (
    event.type === "tribute.resolved" &&
    "match" in event.payload &&
    "hands" in event.payload
  )
    return {
  ...
  if (!result.ok)
    throw new TableSaveError(`stored action is invalid: ${result.code}`);
  return {
    game: {
      ...state.game,
      state: result.state,
      publicEvents: [...state.game.publicEvents, event],
    },
...
function assertCompatible(save: RestorableTableSave): void {
  ...
  if (
    ...
    throw new TableSaveError(
      "rules version mismatch; migration is not available",
    );
  }
  if (save.snapshot.eventSequence !== save.stream.events.length - 1) {
    throw new TableSaveError(
      "snapshot must cover the complete P1 event stream",
    );
  }
}
...
function createSessionFromGame(
  ...
  return {
    ...session,
    snapshot: createSnapshot(stream, snapshotState(session)),
  };
}
...
export function applyTableSessionAction(
  ...
  const stream = appendEvent(
    session.stream,
    eventFor(session.stream.events.length, action),
  );
  const game = {
    ...session.game,
    state: result.state,
...
function cardsForHands(game: TableGame) {
  const cards = (seat: import("../../platform/types").Seat) =>
    game.state.hands[seat]
      ...
      .filter(
        (card): card is import("../../platform/types").Card =>
          card !== undefined,
      );
  return {
    east: cards("east"),
...
function gameWithHands(
  ...
  hands: Readonly<
    Record<
      import("../../platform/types").Seat,
      readonly import("../../platform/types").Card[]
    >
  >,
...
function leaderAfterTribute(
  ...
  if (!firstCard || !secondCard)
    throw new TableSaveError("tribute card is missing");
  const comparison =
...
function advanceAutomatedTribute(session: TableSession): TableSession {
  ...
  if (match.tributePhase === "awaiting-return") {
    ...
    for (const obligation of match.tributePlan.obligations) {
      ...
      if (!choice)
        throw new TableSaveError("recipient has no legal return card");
      submittedReturns.push({
        from: obligation.to,
        to: obligation.from,
        cardId: choice.id,
      });
    ...
    match = {
      ...match,
      submittedReturns,
      tributePhase: allReturned ? "ready" : "awaiting-return",
    };
  }
...
export function getSouthTributeChoices(
  ...
  if (
    !obligation ||
    session.match.submittedTributes.includes(obligation.cardId)
  )
    return [];
...
export function submitSouthTribute(
  session: TableSession,
  cardId: string,
): TableSession {
  const obligation = session.match.tributePlan.obligations.find(
...
export function submitSouthReturn(
  session: TableSession,
  cardId: string,
): TableSession {
  const obligation = session.match.tributePlan.obligations.find(
...
function prepareNextTableSessionWithRoundSeed(
  ...
  if (!session.game.state.completed || finish.length !== 4)
    throw new TableSaveError(
      "cannot prepare next round before the current round completes",
    );
...
export function restartCurrentTableSession(
  ...
  const game = createTableGame(roundSeed, {
    leader,
    levelRank: session.match.levelRank,
  });
  const tributePlan = session.match.previousFinish
    ? createTributePlan(
        session.match.levelRank,
        session.match.previousFinish,
        cardsForHands(game),
      )
    : noTributePlan();
  const match: MatchSessionState = {
...
function restoredDisplayOrder(
  ...
  if (
    !Array.isArray(value) ||
    !value.every((cardId) => typeof cardId === "string")
  )
    return undefined;
...
export function restoreTableSession(save: RestorableTableSave): TableSession {
  ...
  if (
    JSON.stringify(replayed.game.state) !==
      JSON.stringify(save.snapshot.state.state) ||
  ...
  return {
    ...
    humanDisplayOrder: restoredDisplayOrder(
      save as TableSave,
      replayed.game.state.hands.south,
    ),
  };
}
