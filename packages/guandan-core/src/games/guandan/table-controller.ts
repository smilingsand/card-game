...
export function getSelectedPlayActions(
  ...
  const recognition = recognizePatterns(
    cards,
    game.levelRank ?? INITIAL_LEVEL_RANK,
  );
...
export function inspectTableNormalVNext(
  game: TableGame,
): NormalVNextPreviewDiagnostic | undefined {
  ...
  const opponentCounts = Object.entries(view.remainingCardCounts)
    .filter(
      ([seat]) =>
        seat !== view.selfSeat &&
        seat !==
  ...
  return {
    ...
    structureDamageCost: cost?.structureDamageCost ?? 0,
    controlResourceCost: cost?.controlResourceCost ?? 0,
    wildcardOpportunityCost: cost?.wildcardOpportunityCost ?? 0,
    contest:
      view.highestSeat === undefined
        ? "lead"
        : Math.min(...opponentCounts) <= 3
          ? "block"
          : "conserve",
    alerts: diagnoseNormalVNextAction(view, decision.action),
  };
}

export function chooseTableStrategicDecision(game: TableGame) {
  return chooseNormalVNextBotAction(
    createTableBotView(game, getLegalBotActions(game)),
  );
}
