...
function hasNonBombCards(hand: BotView["selfHand"]): boolean {
  return hand.some(
    (card) =>
      card.suit === "joker" ||
      hand.filter((item) => item.rank === card.rank && item.suit !== "joker")
        .length < 4,
  );
}

function splitGroupPenalty(
  action: TurnAction,
  ...
  return action.cardIds.reduce((penalty, cardId) => {
    ...
    const sameRank = hand.filter(
      (item) => item.rank === card.rank && item.suit !== "joker",
    );
    const selectedOfRank = action.cardIds.filter(
      (selectedId) =>
        hand.find((item) => item.id === selectedId)?.rank === card.rank,
    );
    return (
      penalty +
      (sameRank.length >= 2 && selectedOfRank.length < sameRank.length
        ? 500
        : 0)
    );
  }, 0);
...
function wildcardDownrankPenalty(
  action: TurnAction,
  view: BotView,
  endgame: boolean,
): number {
  ...
  return Object.entries(action.interpretation.wildcardAs).some(
    ([cardId, assigned]) => {
      const card = view.selfHand.find((item) => item.id === cardId);
      return (
        card?.suit === "hearts" &&
        card.rank === view.levelRank &&
        assigned.rank !== view.levelRank
      );
    },
  )
    ? 10_000
...
function hasExactGroup(
  ...
  return allowedRanks.some(
    (rank) =>
      hand.filter((card) => card.rank === rank && card.suit !== "joker")
        .length === count,
  );
}

function hasThreeWithPairRecovery(
  hand: BotView["selfHand"],
  levelRank: string,
): boolean {
...
function hasRequiredRecovery(action: TurnAction, view: BotView): boolean {
  ...
  switch (action.interpretation.type) {
    ...
    case "pair":
      return (
        primaryRank >= 13 || hasExactGroup(hand, 2, ["K", "A", view.levelRank])
      );
    case "triple":
      return (
        primaryRank >= 11 ||
        hasExactGroup(hand, 3, ["J", "Q", "K", "A", view.levelRank])
      );
    case "three-with-pair":
      return (
        primaryRank >= 11 || hasThreeWithPairRecovery(hand, view.levelRank)
      );
    default:
      return true;
  }
...
export function chooseBasicBotAction(view: BotView): TurnAction | undefined {
  ...
  const endgame = Object.values(view.remainingCardCounts).some(
    (count) => count <= 1,
  );
