...
describe("牌桌事件存档", () => {
  test("中断后的事件流和快照可恢复相同牌桌及机器人公开记忆", () => {
    ...
    const restored = restoreTableSession(
      serializeTableSession(advanced.session),
    );

    expect(restored.game.state).toEqual(advanced.session.game.state);
    expect(restored.game.publicEvents).toEqual(
      advanced.session.game.publicEvents,
    );
  ...
  test("旧单局存档被明确拒绝，不会被按连续赛局规则恢复", () => {
    const legacySave = structuredClone(
      serializeTableSession(createTableSession(73)),
    );
    Reflect.set(legacySave, "saveSchemaVersion", 2);

    expect(() => restoreTableSession(legacySave)).toThrow(
      "legacy single-round saves",
    );
  });
  ...
  test("重开本局保留上局结果、等级与级牌，但重新发牌并计算贡牌", () => {
    ...
    const completed = {
      ...
      match: {
        ...initial.match,
        currentFinish: ["north", "south", "west", "east"] as const,
      },
    };
    ...
    expect(restoreTableSession(serializeTableSession(restarted)).match).toEqual(
      restarted.match,
    );
  });
  ...
  test("出牌后恢复的显示顺序会剔除已出实体牌 ID", () => {
    const initial = createTableSession(73);
    const arranged = setHumanDisplayOrder(
      initial,
      [...initial.game.state.hands.south].reverse(),
    );
    ...
    expect(
      restoreTableSession(serializeTableSession(advanced.session))
        .humanDisplayOrder,
  ...
  test("当前轮三名下家均过牌后清轮，最高公开出牌随状态清空", () => {
    ...
    const cleared = applyTableSessionAction(afterNorth.session, {
      type: "pass",
      actor: "west",
    });
    ...
    expect(cleared).toMatchObject({
      ok: true,
      state: { current: "south", highest: undefined },
    });
  });
  ...
  test("南家进贡手动提交，机器人自动还贡并在交换后保留可恢复的下一局", () => {
    ...
    const completed = {
      ...
      match: {
        ...initial.match,
        currentFinish: ["east", "north", "west", "south"] as const,
      },
    };
    ...
    expect(
      restoreTableSession(serializeTableSession(exchanged)).game.state.hands,
    ).toEqual(exchanged.game.state.hands);
  });
  ...
  test("南家收贡时必须手动还不大于十的牌，另一位机器人会自动完成双下还贡", () => {
    ...
    if (!awaitingReturn)
      throw new Error("expected a non-anti-tribute dealt round");
