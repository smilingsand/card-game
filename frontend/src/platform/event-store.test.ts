...
describe("版本化 append-only 事件流", () => {
  test("seed 加事件可完整重建状态，且追加不会修改旧流", () => {
    ...
    expect(replayEvents(counterEventReducer, 99, stream.events)).toEqual({ seed: 99, count: 5 });
  });
  ...
  test("快照是事件锚点，可只回放快照后的连续事件", () => {
    ...
    const firstStream = appendEvent(createEventStream("counter-v1"), firstEvent);
    const snapshot = createSnapshot(firstStream, { seed: 99, count: 2 });

    expect(snapshot.eventSequence).toBe(0);
    expect(replayFromSnapshot(counterEventReducer, snapshot, [secondEvent])).toEqual({
      seed: 99,
      count: 5
    });
  });
  ...
  test("追加后源事件的嵌套载荷变更不会篡改已存事件", () => {
    ...
    const stream = appendEvent(createEventStream<typeof sourceEvent>("counter-v1"), sourceEvent);
  ...
  test("快照深复制并冻结状态，不保留调用方可变引用", () => {
    ...
    const snapshot = createSnapshot(createEventStream("counter-v1"), sourceState);
...
describe("纯动作 reducer 基础", () => {
  test("非法动作保留同一状态引用，且不调用 applyAction", () => {
    ...
    expect(result).toEqual({ valid: false, state, error: { code: "amount_must_be_positive" } });
