...
test("双上/头三/头末分别升3/2/1，打A仅双上获胜", () => {
  expect(settleRound({ level: "9", finish: ["east", "west", "south", "north"] })).toMatchObject({
    level: "Q",
    winner: false
  });
  expect(settleRound({ level: "A", finish: ["east", "west", "south", "north"] })).toMatchObject({
    winner: true
  });
  expect(settleRound({ level: "9", finish: ["east", "south", "west", "north"] })).toMatchObject({
    level: "J"
  });
  expect(settleRound({ level: "9", finish: ["east", "south", "north", "west"] })).toMatchObject({
    level: "10"
  });
});
...
test("单下进贡和还贡产生可重放事件并由末游下家首出", () => {
  ...
  expect(() =>
    settleSingleTribute("2", "east", "north", { id: "t", rank: "A" }, { id: "r", rank: "J" })
...
test("双下较大贡牌者先出，同点由头游下家先出", () => {
  ...
  ).toMatchObject({ nextLeader: "north", events: [{ payload: { kind: "double-tribute" } }] });
  expect(
    settleDoubleTribute(
      "2",
      "east",
