...
export function isRequiredTributeCard(
  ...
  const largest = Math.max(...eligible.map((card) => cardValue(card, levelRank)));
...
export function applyTributeExchange(
  ...
  if (plan.kind === "none") {
    if (returns.length !== 0) throw new TributeExchangeError("anti-tribute cannot return cards");
  ...
  const exchanges = plan.obligations.map((obligation) => {
    if (!isRequiredTributeCard(obligation.cardId, initialHands[obligation.from], levelRank)) {
      throw new TributeExchangeError("tribute must be the largest eligible card");
    }
    ...
    const returnCard = initialHands[returned.from].find((card) => card.id === returned.cardId);
