...
let lastGenerationStatistics: CompleteLegalActionsGenerationStatistics | undefined;
...
function completeLegalActionsCacheKey(input: RuleCompleteLegalActionsInput): string {
  return stableJson({
...
function generateCompleteLegalCandidates(
  ...
  const enumerationMilliseconds = performance.now() - enumerationStarted;
  ...
  const recognitionMilliseconds = performance.now() - recognitionStarted;
  const legalFilterStarted = performance.now();
...
export function createCompleteLeadCatalogIndex(
  ...
  const entries = rootLeadingActions.flatMap((action) => {
    ...
    for (const id of action.cardIds) {
      ...
      if (bit === undefined)
        throw new Error("root leading catalogue contains a card outside its root hand");
      mask |= bit;
    }
    return [{ action, mask }];
  });
  return {
    filter(remainingCardIds: readonly string[]): readonly TurnAction[] {
      ...
      for (const id of remainingCardIds) {
        const bit = bitsByCardId.get(id);
        if (bit === undefined) throw new Error("subhand contains a card outside its root hand");
        remainingMask |= bit;
      }
      return entries
