...
test("固定层级与同型比较", () => {
  expect(compareInterpretations(p("normal-bomb", [6, 3], 6), p("straight-flush", [14], 5))).toEqual(
    { ok: true, result: "greater" }
  );
  expect(
    compareInterpretations(p("straight-flush", [14], 5), p("normal-bomb", [5, 15], 5))
  ).toEqual({ ok: true, result: "greater" });
  expect(compareInterpretations(p("four-jokers", [4], 4), p("normal-bomb", [10, 15], 10))).toEqual({
    ok: true,
    result: "greater"
  });
    ...
    for (const right of set) {
      ...
      if (forward.ok && backward.ok) {
        ...
        expect(backward.result).toBe(
          forward.result === "greater" ? "less" : forward.result === "less" ? "greater" : "equal"
        );
  ...
  for (let low = 0; low < ordered.length; low += 1)
    for (let middle = low + 1; middle < ordered.length; middle += 1)
      for (let high = middle + 1; high < ordered.length; high += 1) {
        expect(compareInterpretations(ordered[middle], ordered[low])).toMatchObject({
          ok: true,
          result: "greater"
        });
        expect(compareInterpretations(ordered[high], ordered[middle])).toMatchObject({
          ok: true,
          result: "greater"
        });
        expect(compareInterpretations(ordered[high], ordered[low])).toMatchObject({
          ok: true,
          result: "greater"
        });
      }
