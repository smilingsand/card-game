...
export function validateAction(state: TurnState, action: TurnAction): TurnResult {
  if (action.actor !== state.current) return { ok: false, code: "not_current_player" };
  if (state.finished.includes(action.actor)) return { ok: false, code: "finished_player" };
  if (action.type === "pass")
    return state.highest ? { ok: true, state } : { ok: false, code: "cannot_pass_lead" };
...
export function applyAction(state: TurnState, action: TurnAction): TurnResult {
  ...
  if (action.type === "play") {
    const hand = state.hands[action.actor].filter((id) => !action.cardIds.includes(id));
    const finished = hand.length === 0 ? [...state.finished, action.actor] : state.finished;
  ...
  return { ok: true, state: { ...state, current: next(action.actor, state.finished), passes } };
}
