...
当前实现仅完成可靠传输与房间投影同步。由于现有 Authority 尚未定义 P3-05 真人 seat 到权威动作主体的映射，真实出牌命令转发须先由主 agent 判断是否新增 ADR/任务边界；本任务不将 `sync` ACK 误称为真人出牌支持。

## P3-05 测试矩阵（in_progress）
