...
| P1-15B | P1-13, P1-14 | 手牌首次自动整理和手动理牌：按确定性牌面分组/排序展示，支持拖拽移动，保存仅限自己的显示顺序。 | 排序、拖拽、刷新恢复和不改变出牌实体 ID 的组件/浏览器测试。 | 首次打开即易读；手动位置可恢复；理牌不改变规则状态、事件流或机器人视图。 | not_started |
| P1-15C | P1-13, P1-15A | 标准桌面方位：北上、南下、东左、西右；真人固定为南座，首局仍由东座领出。 | 四座布局/真人南座/东座机器人首出/轮转组件与浏览器测试。 | 真人位于下方；东座、南座、西座、北座与规则座位一致；首局领出规则不变。 | not_started |
